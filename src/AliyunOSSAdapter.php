...
use League\Flysystem\{Adapter\AbstractAdapter, AdapterInterface, Config, Util};
use OSS\{Core\OssException, OssClient};
...
class AliyunOSSAdapter extends AbstractAdapter
{
     ...
     * @param array $options
     */
    public function __construct(OssClient $client, string $bucket, string $endPoint, bool $ssl, bool $isCname = false, bool $debug = false, string $cdnDomain = '', string $prefix = '', array $options = [])
    {
        $this->debug = $debug;
        $this->client = $client;
        $this->bucket = $bucket;
...
//        }
    ...
    /**
     * @param $functionName string function name : __FUNCTION__
     * @param $error
     */
    protected function logError(string $functionName, $error)
    {
        if ($this->debug) {
